...
def main() -> None:
    ...
    try:
        ...
        complex_card_json = input_dict.get('complex')
        if complex_card_json:
            card = AgentCard.model_validate_json(complex_card_json)
            validate_complex_card(card)

        minimal_card_json = input_dict.get('minimal')
        if minimal_card_json:
            card = AgentCard.model_validate_json(minimal_card_json)
            validate_minimal_card(card)

        print('Validation successful')
    except Exception as e:
        print(f'Failed to validate AgentCards with 0.3.24: {e}', file=sys.stderr)
        sys.exit(1)
